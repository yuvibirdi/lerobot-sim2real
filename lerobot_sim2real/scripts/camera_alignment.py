...
def update_camera(sim_env):
    ...
    sim_env.unwrapped._sensors["base_camera"].camera.set_fovy(sim_env.unwrapped.base_camera_settings["fov"] + fov_offset)

    if len(active_keys) > 0:
        print("current_camera_position", pose.p)
        print("current_camera_fov", sim_env.unwrapped.base_camera_settings["fov"] + fov_offset)
        help_message_printed = False  # Reset the flag when there's movement
    elif not help_message_printed:  # Only print help message if it hasn't been printed yet
        print("press: (w), (a) to move in x, (s), (d) to move in y, (up), (down) to move in z, (left), (right) to change fov")
